  ...
  return (
    <div className='w-full bg-gray-100 p-4'>
        ...
        <div className='w-full h-full bg-white px-4 col-span-4'>
            ...
            <div>
                {
                 products.map((item)=>(
                 <div key={item.id} className='w-full border-b-[1px] border-b-gray-300 p-4 flex item-center gap-6'> 
                <div className='w-full flex items-center justify-between gap-6'>
                 <div className='w-1/5'>
                 <img className='w-full h-44 object-contain' src={item.image} alt="ProductImg"></img>
                </div>
                <div className='w-4/5'>
                   ...
                   <p className='text-base'>
                    ...
                    py-1 text-center drop-shadow-lg rounded-md'>
                        <p>Qty:</p>
                        <p onClick={()=>dispatch(decrementQuantity(item.id))} className='cursor-pointer bg-gray-200 
                        px-1 rounded-md hover:bg-gray-400 duration-300'>-</p>
                        <p>{item.quantity}</p>
                        <p onClick={()=>dispatch(incrementQuantity(item.id))} className='cursor-pointer bg-gray-200 
                        px-1 rounded-md hover:bg-gray-400 duration-300'>+</p>
                    </div>
                    <button onClick={()=>dispatch(deleteItem(item.id))} className='bg-red-500
                     w-36 py-1 rounded-lg text-white mt-2 hover:bg-red-700 active:bg-red-900 
                     duration-300'>
                        Delete Item
                        </button>
                </div>
        ...
        <div className='w-full h-52 bg-white col-span-1 flex flex-col items-center p-4'>
            <div>
                <p className='flex gap-2 items-start text-sm'>
                    ...
                    Your order qualified for FREE Shipping choose this option at  checkout. see details...
                </p>
            </div>
            <div>
            ...
            active:to-yellow-500 duration-200 py-1.5 rounded-md mt-3'> Proceed to Pay</button>
        </div>
    </div>
    ):
    <div  className='flex justify-center items-center gap-4 py-10'>
        ...
        <div className='w-96 p-4 bg-white flex flex-col items-center rounded-md shadow-lg'>
            ...
            active:bg-yellow-700 px-8 py-2 font-titleFont font-semibox text-lg'>
